...
def main():
    ...
    for window_size in WINDOW_SIZES:
        for step_ratio in STEP_SIZES_RATIOS:
            ...
            metrics, reliability, system_metrics = process_with_parameters(input_signal, window_size, step_size)
            avg_reliabilities = reliability.get_average_reliabilities()
            
            result = {
                'window_size': window_size,
                'step_size': step_size,
                'reliabilities': avg_reliabilities,
    ...
    for result in all_results:
        ...
        for metric in metrics:
            print(f"{metric.upper()} Reliability: {result['reliabilities'][metric]:.3f}")
        print(f"Computation time: {result['reliabilities']['computation_time']:.6f} s")
        print(f"Average Memory Usage: {result['system_metrics']['avg_memory_mb']:.2f} MB")
        print(f"Average SNR: {result['system_metrics']['avg_snr_db']:.2f} dB")

        
    
def process_with_parameters(input_signal, window_size, step_size):
    system_metrics = SystemMetrics()
    reliability_tracker = MetricsReliability()
    reliability_tracker.window_size = window_size
    reliability_tracker.step_size = step_size
    ...
    for idx in range(0, len(input_signal)-window_size, step_size):
        ...
        # Process each metric and get reliability
        mnf_value, mnf_rel = calc_mnf_arv_ratio(segment, INITIAL_RATE)
        ima_value, ima_rel = calc_ima_diff(segment, index_25, index_80, index_350)
        emd_values, emd_rel = calc_emd_mdf1_2(segment, INITIAL_RATE)
        fluct_variance, fluct_range_values, fluct_mean_diff_values, fluct_entropy_values, fluct_rel = calc_scaled_fluct_metrics(segment)        
        # dom, dfs, som, pse =      alc_mfdma_metrics(segment)
       
        computation_time += time.time() - start_time
        
        # Store values and reliabilities
        ...
        # m["fluct_entropy_values"].append( fluct_entropy_values)       
        # m["DOM"].append(dom)
        # m["DFS"].append(dfs)
        # m["SOM"].append(som)
        # m["PSE"].append(pse)
        ...
        snr = estimate_signal_noise(segment)
        system_metrics.add_metrics(memory_usage, snr)

        reliability_tracker.add_reliability(mnf_rel, ima_rel, emd_rel, fluct_rel, computation_time/num_windows)
   ...
   # Save normalized plots instead of showing them
    ...
    return m, reliability_tracker, system_metrics
...
class MetricsReliability:
    def __init__(self):
        self.mnf_reliabilities = []
        self.ima_reliabilities = []
        self.emd_reliabilities = []
        self.fluct_reliabilities = []
        self.computation_times = []
        self.window_size = 0
        self.step_size = 0
    
    def add_reliability(self, mnf_rel, ima_rel, emd_rel, fluct_rel, comp_time):
        self.mnf_reliabilities.append(mnf_rel)
        self.ima_reliabilities.append(ima_rel)
        self.emd_reliabilities.append(emd_rel)
        self.fluct_reliabilities.append(fluct_rel)
        self.computation_times.append(comp_time)
    
    def get_average_reliabilities(self):
        return {
            'mnf': np.mean(self.mnf_reliabilities),
            'ima': np.mean(self.ima_reliabilities),
            'emd': np.mean(self.emd_reliabilities),
            'fluct': np.mean(self.fluct_reliabilities),
...
class SystemMetrics:
    ...
    def add_metrics(self, memory_usage, snr):
...
def estimate_signal_noise(segment):
    """Estimates SNR using Welch's method"""
    # Using Welch's method to estimate power spectrum
    freqs, psd = signal.welch(segment, fs=INITIAL_RATE)
    
    # Define signal and noise bands (example ranges)
    signal_mask = (freqs >= 20) & (freqs <= 350)  # Main EMG band
    noise_mask = (freqs >= 0) & (freqs < 20)      # Lower frequencies typically contain noise
