...
def main():
    ...
    for filepath in filepaths:
        ...
        fatigue1 = weighted_sum_fatigue(m)
        plot_fatigue(fatigue1, "Weighted Sum Fatigue")


        calculator = FatigueIndexCalculator()
        fatigue_index = calculator.calculate_fatigue_index(m, window_size=6)  # window_size for smoothing
        contributions = calculator.get_metric_contribution(m)
        print(contributions)
        plt.figure()
        plt.plot(fatigue_index, label="Fatigue Index")


        learner = FatigueLearner(m)
        fatigue_pca = learner.extract_fatigue_indicator(method='pca')
        fatigue_kmeans = learner.extract_fatigue_indicator(method='kmeans')
        visualize_fatigue_indicator(fatigue_pca)
        visualize_fatigue_indicator(learner.extract_fatigue_indicator(method='tsne'))
...
class FatigueLearner:
    ...
    def extract_fatigue_indicator(self, method='pca'):
        """
        ...
        """
        if method == 'pca':
            pca_result, variance_ratio = self.pca_analysis()
...
def visualize_fatigue_indicator(fatigue_indicator):
    """
...
class FatigueIndexCalculator:
    ...
    def calculate_fatigue_index(self, metrics, window_size=None):
        ...
        all_metrics = np.array(list(normalized_metrics.values()))
