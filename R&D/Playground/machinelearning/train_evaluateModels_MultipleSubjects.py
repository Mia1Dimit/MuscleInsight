...
if home == True:
    resultsFolderPath = "C:/Users/John Stivaros/Documents/PersonalProjects/MuscleInsight/MuscleInsight/R&D/Playground/machinelearning/results/"
    modelsFolder = "C:\\Users\\John Stivaros\\Documents\\PersonalProjects\\MuscleInsight\\MuscleInsight\\R&D\\Playground\\machinelearning\\models\\"
else:
    resultsFolderPath = "C:\\Users\\jstivaros\\Documents\\MuscleInsight\\MuscleInsight\\R&D\\Playground\\machinelearning\\results"
    modelsFolder = "C:\\Users\\jstivaros\\Documents\\MuscleInsight\\MuscleInsight\\R&D\\Playground\\machinelearning\\models\\"
...
def choose_multiple_json_files():
    root = tk.Tk()
    root.withdraw()  # Hide the main window

    # Ask for multiple input JSON files
    input_files = filedialog.askopenfilenames(
        title="Select Input JSON Files (multiple selection possible)", 
    ...
    subject_name = resultsFolderPath 
    
    # Ask for multiple output JSON files
    output_files = filedialog.askopenfilenames(
        title="Select Output JSON Files (multiple selection possible)", 
        filetypes=[("JSON files", "*.json")]
    )
    
    if not output_files:
        print("No output files selected.")
        return all_input_data, None, subject_name
    
    # Dictionary to store all output data
    all_output_data = {}
    ...
    for output_file in output_files:
        ...
        # Remove 'person' key if it exists
        if 'person' in output_data:
            people.remove(output_data['person'])
            output_data.pop('person')
        
        # Add to the combined dictionary
        for key, value in output_data.items():
            if key in all_output_data:
                all_output_data[key].extend(value)
            else:
                all_output_data[key] = value.copy()
    
    if len(people) == 0:
        return all_input_data, all_output_data, subject_name
    else:
        raise ValueError("All input files must have the same 'person' key")
...
# Get multiple input and output files
input_m, output_m, subjectName = choose_multiple_json_files()

if input_m is None:
    print("Processing aborted.")
else:
    # Convert input dictionary to matrix
    ...
    if output_m and 'pca_filt' in output_m:
...
def save_models(models_dict, output_dir="saved_models"):
    """
    Save multiple models using joblib.
    ...
    # Dictionary to track saved model paths
    model_paths = {}
    ...
    # Save a manifest file with model paths
